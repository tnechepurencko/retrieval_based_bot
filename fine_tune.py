...
def prepare_jsonl(filename, frac):
    df = pd.read_csv(filename + '.csv', sep='\t').drop('original', axis=1)
    ...
    with open(filename + '.jsonl', 'w', encoding='utf-8') as f:
        f.write(result1)
...
tr_file_id = upload_response_id(training_file)
val_file_id = upload_response_id(validation_file)
